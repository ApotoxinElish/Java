...
public class LinkedList<T> {
    ...
    public T lastK(int k) {
        if (k == 0) {
            return head.element;  // if input is 0, return first element
        }

        T result = null;  // to record the return value
        Node<T> cur = head;  // record the node

        for (int index = 0; cur != null; index++) {  // to record the index
            if (index % k == 0) {  // multiple of k
                result = cur.element;
            }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(list.lastK(3));
    }
}
